...
def upload_image():
	...
	if form.validate_on_submit(): # Check if image was uploaded
		...
		prev_image = loader
		snake_name,poisonous,description,  scientific  = image_recognition(loader) # Run image recognition function in imagedetection.py
	else:
		file_url = None
	if snake_name is not None:
		return render_template('index.html', form=form, file_url=file_url, snake_name=snake_name, poisonous=poisonous, description =description,  scientific =scientific)
	else:
